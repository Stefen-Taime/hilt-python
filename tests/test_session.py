...
def test_session_local_backend_with_columns(temp_hilt_file: Path):
    ...
    with Session(backend="local", filepath=temp_hilt_file, columns=custom_columns) as session:
...
def test_session_local_backend_excludes_message_column(temp_hilt_file: Path):
    ...
    with Session(backend="local", filepath=temp_hilt_file, columns=columns_no_message) as session:
        event = Event(
            ...
            metrics=Metrics(
                tokens={"prompt": 50, "completion": 100, "total": 150}, cost_usd=0.00234
            ),
...
def test_session_local_backend_read_filtered_events(temp_hilt_file: Path):
    ...
    read_session = Session(
        backend="local", filepath=temp_hilt_file, mode="r", columns=custom_columns
    )
...
def test_session_local_backend_multiple_columns_combinations(temp_hilt_file: Path):
    ...
    for columns in test_cases:
        ...
        with Session(backend="local", filepath=temp_hilt_file, columns=columns) as session:
            event = Event(
                session_id="test",
                actor=Actor(type="human", id="user"),
                action="prompt",
                ...
                metrics=Metrics(
                    tokens={"prompt": 10, "completion": 20, "total": 30}, cost_usd=0.00123
                ),
                extensions={"model": "gpt-4o-mini"},
            )
            session.append(event)
...
def test_session_local_backend_metrics_extraction(temp_hilt_file: Path):
    ...
    with Session(backend="local", filepath=temp_hilt_file, columns=columns) as session:
        event = Event(
            ...
            action="completion",
            metrics=Metrics(
                tokens={"prompt": 150, "completion": 300, "total": 450}, cost_usd=0.005678
            ),
        )
        session.append(event)
...
def test_session_local_vs_sheets_column_parity(temp_hilt_file: Path):
    ...
    with pytest.raises(ValueError, match="Invalid columns"):
        Session(backend="local", filepath=temp_hilt_file, columns=["invalid_column"])
